...
class SequenceTests {
    ...
    fun `Parse valid sequence`() {
        SequenceTestsFunctions.`Parse valid sequence`(JsonParserDynamicAndUnsafe)
    }
    ...
    fun `Parsing a sequence calculates lazily`() {
        ...
        val cakeSequenceIterator = JsonParserDynamicAndUnsafe.parseSequence<SequenceCake>(json).iterator()
